              ...
              case 6:
                ...
                return actionOnFilterables(modelName, payload, function (joinCollectionName, filterableKey, filterableValue) {
                  if (pathObject.hasOwnProperty(filterableKey) && pathObject[filterableKey] != filterableValue) {
                    return Promise.all([firebase.ref(getPathString(joinCollectionName, pathObject[filterableKey], pathRef.key)).remove(), firebase.ref(getPathString(joinCollectionName, filterableValue, pathRef.key)).set(true)]);
                  } else {
                    return firebase.ref(getPathString(joinCollectionName, filterableValue, pathRef.key)).set(true);
                  }
                });
